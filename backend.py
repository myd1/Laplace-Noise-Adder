...
def getstats(graph):
	...
	if not isEmpty(graph):
		...
		gs = globalSensitivity(graph)
		degree_list = []
		for key,value in nx.degree(graph):
			degree_list.append(value)
		avgDegree = sum(degree_list)/len(degree_list)
	mainstring = "Stats:\n"
	mainstring += "No of Nodes : "+str(noOfNodes)+"\n"
	mainstring += "No of Edges : "+str(noOfEdges)+"\n"
	mainstring += "Global Sensitivity : "+str(gs)+"\n"
	mainstring += "Average Degree : {0:.4f} "
	return mainstring , gs , avgDegree
...
def isEmpty(graph):
	return (list(graph.nodes) == [] and list(graph.edges) == [])